...
- **Documents that already have text.** Pick a document and its current content appears on the page, locked. You can't change it from SyncStream; edit it in Google Docs instead. A blue bar marks your text. A new document opens in choosing mode: hover it to see where your text can go, and click to put it there. Press Esc to just read the document, and use Choose spot at the right end of the toolbar to move your text again. Select all selects only your text. When the sync runs, your text is typed at that spot as new paragraphs. The worker finds its place again before every edit, so changes made elsewhere in the document while it runs don't throw it off. If the document changed between opening it and starting, SyncStream reloads it and asks you to check the spot again.
